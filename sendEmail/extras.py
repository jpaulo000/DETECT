...
def getSubject(messageType):
    if messageType == 0: 
        subject = 'Servidor Azure: arquivo gerado com sucesso'    
    elif messageType == 1:
        subject = 'Situacao do servidor'  
    else:
        subject = 'Erro no servidor Azure'
...
def getMessage(messageType, var):
    if messageType == 0:        
        message = ('Ola\n' + 
               'O servidor na Azure acaba de criar um novo arquivo totalizando ' + str(os.path.getsize(var)) + 'bytes.' + 
               '\n\n' + 
        ...
        message = ('Ola\n' + 
               'O servidor na Azure permaneceu inativo por um periodo de ' + str(var) + ' (hmm). Apesar disto,' + 
